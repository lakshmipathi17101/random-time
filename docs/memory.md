...
## Current Status: Phases 1–6 Complete · Phase 7 landed · Phase 10 mostly complete · Phase 11.0 prebuild landed · Phase 11.1 permission plumbing landed (Kotlin + JS hook + __DEV__ smoke card)
...
- 2026-05-16 — Phase 11.1 permission plumbing landed. New Kotlin module `appcontrol/AppControlModule.kt` exposes `getPermissionStatus()` + 3 deep-link `requestX()` methods (UsageStats / Overlay / Accessibility) via React bridge. Wired through `AppControlPackage.kt` in MainApplication's getPackages() list. Manifest declares `PACKAGE_USAGE_STATS` + `QUERY_ALL_PACKAGES` with `tools:ignore` (must be flavor-gated before Play Store). Proguard `-keep` rule for the appcontrol package. JS side: `nativeAppControl.ts` now detects + dispatches to `NativeModules.AppControl` when present (cached); `hooks/useAppControl.ts` provides a React wrapper with mount-safe state + AppState-on-resume auto-refresh; App.tsx has a `__DEV__`-gated debug smoke card so the user can verify the Settings round-trip on device. `engineering:code-review` skill flagged 3 critical / 8 suggestion items; addressed proguard + AppState dedup + double-refresh inline; rest deferred to 11.2/11.3/11.5. All 161 tests still pass, `tsc --noEmit` clean.
